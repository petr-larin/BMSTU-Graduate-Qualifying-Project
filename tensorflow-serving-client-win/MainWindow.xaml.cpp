...
namespace winrt::tfs_client::implementation
{
    MainWindow::MainWindow()
    {
        ...
        SetWindowPos(win32_hwnd, NULL, 0, 0,
            ...
            int(377 * GetDisplayScaleFactorY()) + wndRect.bottom - wndRect.top - cliRect.bottom,
            SWP_NOMOVE);

        numbox_f1().Value(1500);
        numbox_f4().Value(3.8);
        numbox_f7().Value(5000);
    ...
    void MainWindow::ContactServer(bool get_prediction)
    {
        ...
        if (get_prediction)
            if (response.status_code == 200)
            {
                ...
                //textbox_f2().Text(to_hstring(f2));
                //textbox_f3().Text(to_hstring(f3));
                //textbox_f5().Text(to_hstring(f5));
                //textbox_f6().Text(to_hstring(f6));
                //textbox_rul().Text(to_hstring(rul));
            }
            else
            {
                ClearPredictions();
